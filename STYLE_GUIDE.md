...
> **Scope**: This style guide applies to the **base website home page only** (the landing page at `/`). It is **not** intended for individual apps built as sub-projects of this site — those apps are free to use their own design languages.

This file is the canonical prompt used to design and maintain the base website. When asking Claude Code to redesign, refactor, or extend the landing page, paste or reference this entire document so the agent has the role, design philosophy, tokens, component styles, and layout rules in one place.
